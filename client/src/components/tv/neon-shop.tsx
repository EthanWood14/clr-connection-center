...
export function NeonShop({ data, loading, error, purchaseError, notice, pendingId, onBuy, onRetry }: {
  ...
  const items = (data?.catalog ?? []).filter(i => (tier === "All" || i.tier === tier) && (currency === "all" || i.currency === currency) && (collection === "all" || (collection === "owned" ? i.owned : i.affordable)) && `${i.name} ${i.description}`.toLowerCase().includes(search.toLowerCase()));
  return <section className="neon-shop" aria-label="Garage shop" data-testid="tv-car-shop">
    ...
    <div className="neon-shop-body">
      ...
      <div className="neon-filters"><label className="neon-search"><Search size={16}/><input aria-label="Search upgrades" placeholder="Find your upgrade…" value={search} onChange={e=>setSearch(e.target.value)}/></label><select aria-label="Currency" value={currency} onChange={e=>setCurrency(e.target.value as typeof currency)}><option value="all">All currencies</option>{currencies.map(c=><option key={c} value={c}>{shopCurrencyLabel(c)}</option>)}</select><select aria-label="Collection" value={collection} onChange={e=>setCollection(e.target.value)}><option value="all">All items</option><option value="affordable">Ready to buy</option><option value="owned">My collection</option></select></div>
      ...
      {purchaseError && <p role="alert" className="neon-error">Purchase failed: {purchaseError}</p>}
