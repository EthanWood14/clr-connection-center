...
type LoSplitWindow = "today" | "week" | "month" | "all";
const LO_SPLIT_OPTIONS: { key: LoSplitWindow; label: string }[] = [
  { key: "today", label: "Today" },
  { key: "week", label: "Week" },
  { key: "month", label: "Month" },
  { key: "all", label: "All time" },
];
type LoSplitRow = { loId: number; name: string; helper: number; others: number; total: number };
type LoSplitResp = {
  helperName: string;
...
function TransferScorecard({ rows, rangeLabel, pace }: {
  ...
  const cols: Array<{
    ...
    { key: "placement",    label: "Priority",  get: r => r.placementScore ?? null, better: true,
      ...
      title: "A weighted routing score, NOT the percentage sent to LOs marked priority at transfer time. Transfers to explicitly prioritised LOs score 100% regardless of workload, using saved priority status. Historical priority changes are recovered from audit history where available; older unknown records use a frozen baseline from the rollout, not verified historical status. Changing priority flags later does not change those saved decisions. The active roster still comes from current settings. Ordinary LO workload is reconstructed for the morning it was made. The lightest few receiving LOs score 100%, the busiest 0%, and the rest fall between. An explicit priority designation applies even without an assistant. Investment/2nd Home transfers must record Chris's Justin, Mateo or John: a matching assistant scores 60–100% by workload; a different assistant, or no assistant recorded at all, scores zero. Ordinary transfers are compared with the WHOLE floor, not a borrower-state licensing pool. Longer ranges use a fortnight PLUS the range, not a rolling fortnight, so scores can differ by selected range. Unresolved recipients can be counted at the floor average. Fewer than five readable transfers shows a dash. If the roster cannot resolve the investment assistants, those transfers are scored as ordinary placement instead and the cell notes routing rule off. Hover a cell for its breakdown.",
...
export default function ManagerDashboard({ view = "advanced" }: { view?: "advanced" | "overview"; params?: unknown } = {}) {
  ...
  const [loSplitWindow, setLoSplitWindow] = useState<LoSplitWindow>("today");
